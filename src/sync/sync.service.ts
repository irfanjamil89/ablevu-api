...
export class SyncService {
    ...
    private readonly accessiblecityApiUrl = 'https://ablevu.com/api/1.1/obj/accessiblecity';
    private readonly businessAccessibilityMediaApiUrl = 'https://ablevu.com/api/1.1/obj/BusinessAccessibilityMedia';
    private readonly businessAccessibilityQuestionsApiUrl = 'https://ablevu.com/api/1.1/obj/BusinessAccessibilityQuestion';
    private readonly businessAdditionalResourcesApiUrl = 'https://ablevu.com/api/1.1/obj/BusinessAdditionalResources';
    private readonly businessAudioToursApiUrl='https://ablevu.com/api/1.1/obj/BusinessAudioTour';
    private readonly businessCustomMediaApiUrl = 'https://ablevu.com/api/1.1/obj/BusinessCustomMedia';
    private readonly businessCustomSectionApiUrl= 'https://ablevu.com/api/1.1/obj/BusinessCustomSection';
    private readonly businessImagesApiUrl='https://ablevu.com/api/1.1/obj/BusinessImage';
    private readonly partnerApiUrl='https://ablevu.com/api/1.1/obj/Partner';
    private readonly businessPartnersApiUrl='https://ablevu.com/api/1.1/obj/BusinessPartners';
    private readonly businessRecommendationsApiUrl='https://ablevu.com/api/1.1/obj/BusinessRecommendations';
    private readonly businessScheduleApiUrl='https://ablevu.com/api/1.1/obj/BusinessSchedule';
    private readonly feedbackApiUrl='https://ablevu.com/api/1.1/obj/Feedback';
    private readonly businessReviewsApiUrl='https://ablevu.com/api/1.1/obj/businessReview';
    private readonly couponsApiUrl='https://ablevu.com/api/1.1/obj/CouponCode';
    private readonly subscriptionsApiUrl='https://ablevu.com/api/1.1/obj/Subscription';
  ...
  async SyncAC() {
    ...
    const response = await axios.get<any>(`${this.accessiblecityApiUrl}?cursor=0&limit=100`, {
        headers: {
            Authorization: `Bearer ${this.apiToken}`,
        },
    });
    ...
    for (const bubbleCity of bubbleCities) {
        try {
            ...
            const user = await this.userService.findOneByExternalId(bubbleCity['Created By']);
            
            if (!user || !user.id) {
                console.error(`User not found for external_id: ${bubbleCity['Created By']} - Skipping city: ${bubbleCity.city_text}`);
                continue;  // Skip this city if user doesn't exist
            }

            let userId = user.id;
            let existingCity = await this.accessibleCityService.findByExternalId(bubbleCity._id);
            ...
            if (Array.isArray(businessList) && businessList.length > 0) {
                for (const businessExternalId of businessList) {
                    try {
                        const business = await this.businessService.findByExternalId(businessExternalId);
                        if (business) {
            ...
            if (cityDto.business_Ids.length === 0) {
                console.log(`City ${bubbleCity.city_text} has no businesses yet - will be added later`);
            }
            ...
            if (existingCity) {
                ...
                await this.accessibleCityService.createAccessibleCity(userId, cityDto, bubbleCity._id);
...
async SyncBusinessesAM() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business accessibility media with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessAccessibilityMediaApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessMediaItems: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleMedia of businessMediaItems) {
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleMedia.businesslinked_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleMedia.businesslinked_custom_business} not found. Skipping media ${bubbleMedia._id}.`);
                    continue;
                }
                console.log(`Start Syncing Media ${bubbleMedia._id}, ${bubbleMedia.mediatitle_text} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleMedia['Created By']))?.id;
                ...
                if (existingMedia) {
                    ...
                    console.log(`✅ Media already exists - Updating: ${bubbleMedia.mediatitle_text}`);
                    ...
                    // Create new media
                    console.log(`Creating new media: ${bubbleMedia.mediatitle_text} for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business media from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business media data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
async SyncBusinessesAQ() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business questions with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessAccessibilityQuestionsApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessQuestions: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleQuestion of businessQuestions) {
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleQuestion.business_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleQuestion.business_custom_business} not found. Skipping question ${bubbleQuestion._id}.`);
                    continue;
                }
                console.log(`Start Syncing Question ${bubbleQuestion._id}, ${bubbleQuestion.question_text} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleQuestion['Created By']))?.id;
                ...
                if (existingQuestion) {
                    ...
                    console.log(`✅ Question already exists - Updating: ${bubbleQuestion.question_text}`);
                    ...
                    existingQuestion.active = bubbleQuestion['approved?_boolean'] ?? true;
                    existingQuestion.show_name = bubbleQuestion['showName?_boolean'] ?? true;
                    if (creatorUserId) {
                    ...
                    // Create new question
                    console.log(`Creating new question: ${bubbleQuestion.question_text} for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business questions from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business questions data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncBusinessesAR() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business resources with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessAdditionalResourcesApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessResources: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleResource of businessResources) {
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleResource.busiensslinked_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleResource.busiensslinked_custom_business} not found. Skipping resource ${bubbleResource._id}.`);
                    continue;
                }
                console.log(`Start Syncing Resource ${bubbleResource._id}, ${bubbleResource.title_text} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleResource['Created By']))?.id;
                ...
                if (existingResource) {
                    ...
                    console.log(`✅ Resource already exists - Updating: ${bubbleResource.title_text}`);
                    ...
                    existingResource.active = bubbleResource['approved?_boolean'] ?? true;
                    if (creatorUserId) {
                    ...
                    // Create new resource
                    console.log(`Creating new resource: ${bubbleResource.title_text} for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business resources from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business resources data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncBusinessesAT() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business audio tours with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessAudioToursApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessAudioTours: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleAudioTour of businessAudioTours) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Audio Tour Fields:', JSON.stringify(bubbleAudioTour, null, 2));
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleAudioTour.businessid_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleAudioTour.businessid_custom_business} not found. Skipping audio tour ${bubbleAudioTour._id}.`);
                    continue;
                }
                
                // Use filename if available, otherwise create default name
                // FIXED: Ensure we always have a valid name (never null/undefined)
                const audioTourName = bubbleAudioTour.filename_text && bubbleAudioTour.filename_text.trim() !== '' 
                    ? bubbleAudioTour.filename_text.trim()
                    : `Audio Tour - ${bubbleAudioTour._id}`;
                
                console.log(`Start Syncing Audio Tour ${bubbleAudioTour._id}, ${audioTourName} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleAudioTour['Created By']))?.id;
                ...
                if (existingAudioTour) {
                    ...
                    console.log(`✅ Audio Tour already exists - Updating: ${audioTourName}`);
                    ...
                    existingAudioTour.active = bubbleAudioTour.approved__boolean ?? true;
                    if (creatorUserId) {
                    ...
                    console.log(`Creating new audio tour: ${audioTourName} for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business audio tours from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business audio tours data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
async SyncBusinessesCSM() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business custom section media with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessCustomMediaApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessCustomMedia: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleCustomMedia of businessCustomMedia) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Custom Media Fields:', JSON.stringify(bubbleCustomMedia, null, 2));
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleCustomMedia.businesslinked_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleCustomMedia.businesslinked_custom_business} not found. Skipping custom media ${bubbleCustomMedia._id}.`);
                    continue;
                }
                
                // Use title if available, otherwise create default title
                const mediaTitle = bubbleCustomMedia.mediatitle_text && bubbleCustomMedia.mediatitle_text.trim() !== '' 
                    ? bubbleCustomMedia.mediatitle_text 
                    : `Custom Media - ${bubbleCustomMedia._id}`;
                
                console.log(`Start Syncing Custom Media ${bubbleCustomMedia._id}, ${mediaTitle} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleCustomMedia['Created By']))?.id;
                ...
                if (bubbleCustomMedia.setionlinked_custom_customsection) {
                    const customSection = await this.businessCustomSectionRepo.findOne({
                        where: {
                            external_id: bubbleCustomMedia.setionlinked_custom_customsection
                        }
                    });
                    if (customSection) {
                        customSectionId = customSection.id;
                    } else {
                        console.log(`⚠️ Custom section with external_id ${bubbleCustomMedia.setionlinked_custom_customsection} not found`);
                    }
                }

                // Check if this custom media already exists by external_id
                const existingCustomMedia = await this.businessCustomSectionMediaRepo.findOne({
                    where: {
                ...
                if (existingCustomMedia) {
                    ...
                    console.log(`✅ Custom Media already exists - Updating: ${mediaTitle}`);
                    ...
                    existingCustomMedia.description = bubbleCustomMedia.mediadescription_text || null;
                    existingCustomMedia.active = bubbleCustomMedia.approved__boolean ?? true;
                    if (creatorUserId) {
                    ...
                    console.log(`Creating new custom media: ${mediaTitle} for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business custom media from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business custom media data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncBusinessesCS() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business custom sections with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessCustomSectionApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessCustomSections: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleCustomSection of businessCustomSections) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Custom Section Fields:', JSON.stringify(bubbleCustomSection, null, 2));
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleCustomSection.businesslinked_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleCustomSection.businesslinked_custom_business} not found. Skipping custom section ${bubbleCustomSection._id}.`);
                    continue;
                }
                
                // Use title if available, otherwise create default title
                const sectionTitle = bubbleCustomSection.sectiontitle_text && bubbleCustomSection.sectiontitle_text.trim() !== '' 
                    ? bubbleCustomSection.sectiontitle_text 
                    : `Custom Section - ${bubbleCustomSection._id}`;
                
                console.log(`Start Syncing Custom Section ${bubbleCustomSection._id}, ${sectionTitle} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleCustomSection['Created By']))?.id;

                // Check visibility - convert "Yes"/"No" to boolean
                const isVisible = bubbleCustomSection.visible__text?.toLowerCase() === 'yes';

                // Check if this custom section already exists by external_id
                const existingCustomSection = await this.businessCustomSectionRepo.findOne({
                    where: {
                ...
                if (existingCustomSection) {
                    ...
                    console.log(`✅ Custom Section already exists - Updating: ${sectionTitle}`);
                    ...
                    console.log(`Creating new custom section: ${sectionTitle} for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business custom sections from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business custom sections data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncBusinessesImages() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business images with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessImagesApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessImages: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleImage of businessImages) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Image Fields:', JSON.stringify(bubbleImage, null, 2));
                
                // Skip if name/imagetitle is empty (required field - no default)
                if (!bubbleImage.imagetitle_text || bubbleImage.imagetitle_text.trim() === '') {
                    console.log(`Skipping image ${bubbleImage._id} - imagetitle is empty`);
                    continue;
                }
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleImage.businesslinked_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleImage.businesslinked_custom_business} not found. Skipping image ${bubbleImage._id}.`);
                    continue;
                }
                
                console.log(`Start Syncing Image ${bubbleImage._id}, ${bubbleImage.imagetitle_text} for business ${existingBusiness.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleImage['Created By']))?.id;

                // Convert tags array to comma-separated string
                let tagsString: string | null = null;
                if (Array.isArray(bubbleImage.imagetags_list_text) && bubbleImage.imagetags_list_text.length > 0) {
                    tagsString = bubbleImage.imagetags_list_text.join(', ');
                }

                // Check if this image already exists by external_id
                const existingImage = await this.businessImagesRepo.findOne({
                    where: {
                        external_id: bubbleImage._id,
                    }
                });

                if (existingImage) {
                    // Update existing image
                    console.log(`✅ Image already exists - Updating: ${bubbleImage.imagetitle_text}`);
                    existingImage.business_id = existingBusiness.id;
                    existingImage.name = bubbleImage.imagetitle_text;
                    existingImage.description = bubbleImage.imagedescription_text || null;
                    // existingImage.tags = tagsString;
                    existingImage.image_url = bubbleImage.properyimage_image || null;
                    existingImage.active = bubbleImage.approved__boolean ?? true;
                    if (creatorUserId) {
                        existingImage.modified_by = creatorUserId;
                    }
                    existingImage.modified_at = new Date();
                    await this.businessImagesRepo.save(existingImage);
                    console.log(`✅ Updated image: ${bubbleImage.imagetitle_text}`);
                } else {
                    // Create new image
                    console.log(`Creating new image: ${bubbleImage.imagetitle_text} for business ${existingBusiness.name}`);
                    let newImage = this.businessImagesRepo.create({
                        external_id: bubbleImage._id,
                        business_id: existingBusiness.id,
                        name: bubbleImage.imagetitle_text,
                        description: bubbleImage.imagedescription_text || null,
                        // tags: tagsString,
                        image_url: bubbleImage.properyimage_image || null,
                        active: bubbleImage.approved__boolean ?? true,
                        created_by: creatorUserId,
                        modified_by: creatorUserId,
                    });
                    await this.businessImagesRepo.save(newImage);
                    console.log(`✅ Created image: ${bubbleImage.imagetitle_text}`);
                }
            }
        }
    } catch (error) {
        console.error('❌ Error fetching business images from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business images data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncPartners() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching partners with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.partnerApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            for (const bubblePartner of partners) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Partner Fields:', JSON.stringify(bubblePartner, null, 2));
                
                // Skip if name is empty (required field - no default)
                if (!bubblePartner.partnername_text || bubblePartner.partnername_text.trim() === '') {
                    console.log(`Skipping partner ${bubblePartner._id} - partnername is empty`);
                    continue;
                }
                
                console.log(`Start Syncing Partner ${bubblePartner._id}, ${bubblePartner.partnername_text}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubblePartner['Created By']))?.id;
                ...
                if (existingPartner) {
                    ...
                    console.log(`✅ Partner already exists - Updating: ${bubblePartner.partnername_text}`);
                    existingPartner.name = bubblePartner.partnername_text;
                    existingPartner.description = bubblePartner.description_text || null;
                    ...
                    console.log(`✅ Updated partner: ${bubblePartner.partnername_text}`);
                } else {
                    // Create new partner
                    console.log(`Creating new partner: ${bubblePartner.partnername_text}`);
                    ...
                    console.log(`✅ Created partner: ${bubblePartner.partnername_text}`);
                }
            }
        }
    } catch (error) {
        console.error('❌ Error fetching partners from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble partners data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncBusinessPartners() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business partners with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessPartnersApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessPartners: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleBusinessPartner of businessPartners) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Business Partner Fields:', JSON.stringify(bubbleBusinessPartner, null, 2));
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleBusinessPartner.linkedbusiness_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleBusinessPartner.linkedbusiness_custom_business} not found. Skipping business partner ${bubbleBusinessPartner._id}.`);
                    continue;
                }
                ...
                if (!existingPartner) {
                    console.log(`Partner with external_id ${bubbleBusinessPartner.partner_custom_partner} not found. Skipping business partner ${bubbleBusinessPartner._id}.`);
                    continue;
                }
                
                console.log(`Start Syncing Business Partner ${bubbleBusinessPartner._id} for business ${existingBusiness.name} and partner ${existingPartner.name}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleBusinessPartner['Created By']))?.id;

                // Check if this business partner link already exists by external_id
                const existingBusinessPartner = await this.businessPartnersRepo.findOne({
                    where: {
                ...
                if (existingBusinessPartner) {
                    ...
                    console.log(`✅ Business Partner already exists - Updating for business ${existingBusiness.name}`);
                    ...
                    existingBusinessPartner.active = bubbleBusinessPartner.approved__boolean ?? true;
                    if (creatorUserId) {
                    ...
                    console.log(`Creating new business partner link for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business partners from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business partners data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
async SyncBusinessRecommendations() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business recommendations with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessRecommendationsApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessRecommendations: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleRecommendation of businessRecommendations) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Recommendation Fields:', JSON.stringify(bubbleRecommendation, null, 2));
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleRecommendation.linkedbusiness_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleRecommendation.linkedbusiness_custom_business} not found. Skipping recommendation ${bubbleRecommendation._id}.`);
                    continue;
                }

                // Find the user by external_id (optional - for logging)
                let existingUser = await this.userService.findOneByExternalId(bubbleRecommendation.linkeduser_user);
                const userEmail = existingUser ? existingUser.email : 'Unknown User';
                
                console.log(`Start Syncing Recommendation ${bubbleRecommendation._id} for business ${existingBusiness.name} by user ${userEmail}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleRecommendation['Created By']))?.id;

                // Check if this recommendation already exists by external_id
                const existingRecommendation = await this.businessRecommendationsRepo.findOne({
                    where: {
                ...
                if (existingRecommendation) {
                    ...
                    console.log(`✅ Recommendation already exists - Updating for business ${existingBusiness.name}`);
                    existingRecommendation.business = existingBusiness; // Assign the business entity
                    existingRecommendation.label = bubbleRecommendation.label_text || 'like';
                    ...
                    console.log(`Creating new recommendation for business ${existingBusiness.name}`);
        ...
        console.error('❌ Error fetching business recommendations from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business recommendations data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
async SyncBusinessSchedule() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business schedules with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessScheduleApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessSchedules: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleSchedule of businessSchedules) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Schedule Fields:', JSON.stringify(bubbleSchedule, null, 2));
                
                // Find the business by external_id
                let existingBusiness = await this.businessService.findByExternalId(bubbleSchedule.linkedbusiness_custom_business);
                if (!existingBusiness) {
                    console.log(`Business with external_id ${bubbleSchedule.linkedbusiness_custom_business} not found. Skipping schedule ${bubbleSchedule._id}.`);
                    continue;
                }
                
                console.log(`Start Syncing Schedule ${bubbleSchedule._id} for business ${existingBusiness.name} - ${bubbleSchedule.day_option_days_of_week}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleSchedule['Created By']))?.id;
                ...
                if (existingSchedule) {
                    ...
                    console.log(`✅ Schedule already exists - Updating for ${bubbleSchedule.day_option_days_of_week}`);
                    ...
                    existingSchedule.opening_time = bubbleSchedule.openingtime_date ? new Date(bubbleSchedule.openingtime_date) : existingSchedule.opening_time;
                    existingSchedule.closing_time = bubbleSchedule.closingtime_date ? new Date(bubbleSchedule.closingtime_date) : existingSchedule.closing_time;
                    existingSchedule.opening_time_text = bubbleSchedule.openinghour_text || '';
                    existingSchedule.closing_time_text = bubbleSchedule.closingtime_text || '';
                    ...
                    console.log(`✅ Updated schedule for ${bubbleSchedule.day_option_days_of_week}`);
                } else {
                    // Create new schedule - skip if required date fields are missing
                    if (!bubbleSchedule.openingtime_date || !bubbleSchedule.closingtime_date) {
                        console.log(`⚠️ Skipping schedule for ${bubbleSchedule.day_option_days_of_week} - missing opening or closing time`);
                        continue;
                    }
                    
                    console.log(`Creating new schedule for business ${existingBusiness.name} - ${bubbleSchedule.day_option_days_of_week}`);
                    ...
                    console.log(`✅ Created schedule for ${bubbleSchedule.day_option_days_of_week}`);
                }
            }
        }
    } catch (error) {
        console.error('❌ Error fetching business schedules from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business schedules data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncFeedback() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching feedback with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.feedbackApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const feedbacks: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleFeedback of feedbacks) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Feedback Fields:', JSON.stringify(bubbleFeedback, null, 2));
                ...
                if (bubbleFeedback.feedbacktype_text) {
                    ...
                    if (feedbackType) {
                        ...
                        console.log(`⚠️ Feedback type '${bubbleFeedback.feedbacktype_text}' not found in database for feedback ${bubbleFeedback._id}`);
                    }
                }
                ...
                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleFeedback['Created By']))?.id;
                ...
                if (existingFeedback) {
                    ...
                    if (feedbackTypeId) existingFeedback.feedback_type_id = feedbackTypeId;
                    existingFeedback.comment = bubbleFeedback.comments_text || '';
                    existingFeedback.approved_at = bubbleFeedback.approvedat_date ? new Date(bubbleFeedback.approvedat_date) : new Date();
                    ...
                    if (!feedbackTypeId) {
                        console.log(`⚠️ Skipping feedback ${bubbleFeedback._id} - missing feedback type`);
                        continue;
                    }
                    ...
                    let newFeedback = this.feedbackRepo.create({
                        ...
                        approved_at: bubbleFeedback.approvedat_date ? new Date(bubbleFeedback.approvedat_date) : new Date(),
                        active: true,
                        created_by: creatorUserId,
                        modified_by: creatorUserId,
                    });
        ...
        console.error('❌ Error fetching feedback from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble feedback data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncBusinessReviews() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching business reviews with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.businessReviewsApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const businessReviews: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleReview of businessReviews) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Review Fields:', JSON.stringify(bubbleReview, null, 2));
                
                // Find the business by external_id
                let businessId: string | null = null;
                if (bubbleReview.businesslinked_custom_business) {
                    const existingBusiness = await this.businessService.findByExternalId(bubbleReview.businesslinked_custom_business);
                    if (existingBusiness) {
                        businessId = existingBusiness.id;
                    } else {
                        console.log(`⚠️ Business with external_id ${bubbleReview.businesslinked_custom_business} not found for review ${bubbleReview._id}`);
                    }
                }

                // Find review type by title/name - reviewoptions_x_list_text is an array, use first value
                let reviewTypeId: string | null = null;
                if (Array.isArray(bubbleReview.reviewoptions_x_list_text) && bubbleReview.reviewoptions_x_list_text.length > 0) {
                    ...
                    if (reviewType) {
                        ...
                        console.log(`⚠️ Review type '${reviewTypeTitle}' not found in database for review ${bubbleReview._id}`);
                    }
                }
                ...
                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleReview['Created By']))?.id;
                ...
                if (existingReview) {
                    ...
                    existingReview.description = bubbleReview.reviewdescripion_text || '';
                    existingReview.approved = bubbleReview.recommended__boolean ?? false;
                    existingReview.approvedAt = bubbleReview.approveddate_date ? new Date(bubbleReview.approveddate_date) : new Date();
                    ...
                    if (!businessId || !reviewTypeId) {
                        console.log(`⚠️ Skipping review ${bubbleReview._id} - missing business or review type`);
                        continue;
                    }
                    ...
                    let newReview = this.businessReviewsRepo.create({
                        ...
                        approvedAt: bubbleReview.approveddate_date ? new Date(bubbleReview.approveddate_date) : new Date(),
        ...
        console.error('❌ Error fetching business reviews from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble business reviews data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
...
async SyncCoupons() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching coupons with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.couponsApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            for (const bubbleCoupon of coupons) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Coupon Fields:', JSON.stringify(bubbleCoupon, null, 2));
                
                console.log(`Start Syncing Coupon ${bubbleCoupon._id}, ${bubbleCoupon.code_text}`);

                // Get creator user id
                let creatorUserId = (await this.userService.findOneByExternalId(bubbleCoupon['Created By']))?.id;
                ...
                if (existingCoupon) {
                    ...
                    console.log(`✅ Coupon already exists - Updating: ${bubbleCoupon.code_text}`);
                    existingCoupon.code = bubbleCoupon.code_text || '';
                    existingCoupon.name = bubbleCoupon.name_text || '';
                    existingCoupon.validitymonths = bubbleCoupon.expiry_number?.toString() || '0';
                    existingCoupon.discount = bubbleCoupon.discountamount_number?.toString() || '0';
                    existingCoupon.stripe_coupon_id = bubbleCoupon.stripecouponid_text || null;
                    existingCoupon.stripe_promo_code_id = bubbleCoupon.stripepromocodeid_text || null;
        ...
        console.error('❌ Error fetching coupons from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble coupons data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
async SyncSubscriptions() {
    ...
    try {
        ...
        let remaining = 1;
        while (remaining >= 1) {
            console.log(`Fetching subscriptions with cursor: ${cursor} - ${remaining} remaining`);
            const response = await axios.get<any>(`${this.subscriptionsApiUrl}?cursor=${cursor}&limit=100`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
            });
            ...
            // Normalize potential shapes: { response: { results: [...] } } or { response: [...] }
            const data = response.data as any;
            const subscriptions: any[] = data?.response?.results ?? data?.response ?? [];

            for (const bubbleSubscription of subscriptions) {
                // DEBUG: Print exact field names from Bubble
                console.log('Bubble Subscription Fields:', JSON.stringify(bubbleSubscription, null, 2));
                
                // Find user by external_id
                let userId: string | null = null;
                if (bubbleSubscription.relateduser_user) {
                    const existingUser = await this.userService.findOneByExternalId(bubbleSubscription.relateduser_user);
                    if (existingUser) {
                        userId = existingUser.id;
                    } else {
                        console.log(`⚠️ User with external_id ${bubbleSubscription.relateduser_user} not found for subscription ${bubbleSubscription._id}`);
                    }
                }
                ...
                if (bubbleSubscription.businesslinked_custom_business) {
                    const existingBusiness = await this.businessService.findByExternalId(bubbleSubscription.businesslinked_custom_business);
                    if (existingBusiness) {
                        businessId = existingBusiness.id;
                    }
                }
                ...
                // Skip if no user_id
                if (!userId) {
                    console.log(`⚠️ Skipping subscription ${bubbleSubscription._id} - missing user`);
                    continue;
                }
                ...
                if (existingSubscription) {
                    ...
                    existingSubscription.amount = bubbleSubscription.amount_number?.toString() || '0';
                    existingSubscription.packageName = bubbleSubscription.package1_option_subscription_plan || 'monthly';
                    existingSubscription.priceId = bubbleSubscription.PriceId_text || '';
                    existingSubscription.start_date = bubbleSubscription.startdate_date ? new Date(bubbleSubscription.startdate_date) : undefined;
                    existingSubscription.end_date = bubbleSubscription.enddate_date ? new Date(bubbleSubscription.enddate_date) : undefined;
                    existingSubscription.discount_code = bubbleSubscription.DiscountCode_text || undefined;
                    existingSubscription.discount_amount = bubbleSubscription.DiscountAmount_number?.toString() || '0';
                    existingSubscription.stripe_subscription_id = bubbleSubscription.subid_text || undefined;
                    existingSubscription.status = bubbleSubscription.status_text || 'pending';
                    existingSubscription.payment_reference = bubbleSubscription.PaymentReference_text || undefined;
                    existingSubscription.invoice_id = bubbleSubscription.invoicelink_text || undefined;
                    existingSubscription.success_at = bubbleSubscription.SuccessAt_date ? new Date(bubbleSubscription.SuccessAt_date) : undefined;
                    existingSubscription.cancel_at = bubbleSubscription.CancelAt_date ? new Date(bubbleSubscription.CancelAt_date) : undefined;
                    ...
                    let newSubscription = this.subscriptionsRepo.create({
                        ...
                        packageName: bubbleSubscription.package1_option_subscription_plan || 'monthly',
                        priceId: bubbleSubscription.PriceId_text || '',
                        start_date: bubbleSubscription.startdate_date ? new Date(bubbleSubscription.startdate_date) : undefined,
                        end_date: bubbleSubscription.enddate_date ? new Date(bubbleSubscription.enddate_date) : undefined,
                        discount_code: bubbleSubscription.DiscountCode_text || undefined,
                        discount_amount: bubbleSubscription.DiscountAmount_number?.toString() || '0',
                        stripe_subscription_id: bubbleSubscription.subid_text || undefined,
                        status: bubbleSubscription.status_text || 'pending',
                        payment_reference: bubbleSubscription.PaymentReference_text || undefined,
                        invoice_id: bubbleSubscription.invoicelink_text || undefined,
                        success_at: bubbleSubscription.SuccessAt_date ? new Date(bubbleSubscription.SuccessAt_date) : undefined,
                        cancel_at: bubbleSubscription.CancelAt_date ? new Date(bubbleSubscription.CancelAt_date) : undefined,
                    });
        ...
        console.error('❌ Error fetching subscriptions from Bubble:', (error as any).response?.data || error);
        throw new HttpException('Failed to fetch Bubble subscriptions data', HttpStatus.BAD_REQUEST);
    }
    return response;
}
